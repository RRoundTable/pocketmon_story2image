...
if dataset == '102flowers': # images
    ...
    caption_sub_dir = load_folder_list( caption_dir ) # load_folder_list
    captions_dict = {}
    processed_capts = []
    ...
    for name in imgs_title_list:
        # print("name : ", name)
        img_raw=mpimg.imread(os.path.join(img_dir, name))
        # img_raw = scipy.misc.imread( os.path.join(img_dir, name) ) # int형으로 출력된다. 수정하기
        # plt.imshow(img_raw)
