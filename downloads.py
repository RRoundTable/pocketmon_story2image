...
# get_confirm_token : download의 상태를 파악한다 / key값으로 확인하다 ACK의 역할
def get_confirm_token(response):
	for key, value in response.cookies.items():
		if key.startswith('download_warning'): # download_warning 이라는 str로 시작한다면...
			return value # value를 return한다
	return None
...
def download_file_from_google_drive(id, destination):    
	...
	response = session.get(URL, params={ 'id': id }, stream=True) # response : cookie 대신 새션을 이용한다.
	token = get_confirm_token(response)  # 'download_warning' 으로 시작한다면...
...
def download_caption(dirpath):
	...
	if os.path.exists(os.path.join(dirpath, data_dir)): # dirpath : directory path
...
def download(url, dirpath):
	...
	f = open(filepath, 'wb')
	filesize = int(u.headers["Content-Length"]) # header에서 추출한다
...
def unzip(src_dir,new_name = None): # 압축해체 function
